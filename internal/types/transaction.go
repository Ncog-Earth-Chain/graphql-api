...
func (trx *Transaction) MarshalBSON() ([]byte, error) {
	...
	// DDB commit txs keep their real recipient (the 0x…DDDB sentinel) in storage so the persisted view
	// matches the live RPC view (the node returns To = 0x…DDDB); they are identified via IsDDBTransaction
	// / the Contract.IsDDB flag, not by a nil recipient. (Previously To was nulled here, which made stored
	// DDB txs look like contract-creation and disagree with the live node response.)
