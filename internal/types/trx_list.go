...
type TransactionList struct {
	...
	// TotalIsExact reports whether Total is an exact count or a lower bound.
	// An exact count over a filter that no single index can serve (notably the
	// account page's $or over sender/recipient) can exceed its time budget; in that
	// case the count is capped and this is false.
	TotalIsExact bool
