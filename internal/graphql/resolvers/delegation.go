...
// OutstandingSNEC resolves the amount of outstanding sNEC tokens
// minted for this account.
func (del Delegation) OutstandingSNEC() (hexutil.Big, error) {
	val, err := repository.R().DelegationOutstandingSNEC(&del.Address, del.Delegation.ToStakerId)
	if err != nil {
		return hexutil.Big{}, err
	}
	return *val, nil
