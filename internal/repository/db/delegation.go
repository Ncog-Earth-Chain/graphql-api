...
func (db *PostgreSQLBridge) DelegationsCount() (uint64, error) {
    var count uint64

    // Replace `colDelegations` with your actual table name as a string
    tableName := "delegations"

    // Perform the SQL query to count rows
    query := `SELECT COUNT(*) FROM ` + tableName
    err := db.db.QueryRow(query).Scan(&count)
    if err != nil {
        db.log.Errorf("failed to count delegations: %s", err.Error())
        return 0, err
    }
