...
func (db *MongoDbBridge) initDelegationCollection(col *mongo.Collection) {
	...
	// address + ordinal, so a per-account delegation list is one index scan
	ix = append(ix, mongo.IndexModel{Keys: bson.D{
		{Key: types.FiDelegationAddress, Value: 1},
		{Key: types.FiDelegationOrdinal, Value: -1},
	}})

	// create indexes
	if _, err := col.Indexes().CreateMany(context.Background(), ix); err != nil {
		db.log.Errorf("can not create indexes for delegation collection; %s", err.Error())
	}
