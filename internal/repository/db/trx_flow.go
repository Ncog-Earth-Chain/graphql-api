...
func (db *PostgreSQLBridge) TrxDailyFlowList(from *time.Time, to *time.Time) ([]*types.DailyTrxVolume, error) {
	...
	return loadTrxDailyFlowListPostgres(rows)
}
...
func (db *PostgreSQLBridge) TrxGasSpeed(from *time.Time, to *time.Time) (float64, error) {
	...
	whereClause, args := trxDailyFlowListFilterPostgres(from, to)
...
func trxDailyFlowListFilterPostgres(from *time.Time, to *time.Time) (string, []interface{}) {
...
func loadTrxDailyFlowListPostgres(rows *sql.Rows) ([]*types.DailyTrxVolume, error) {
