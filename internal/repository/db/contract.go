...
func contractListTopFilter(validatedOnly bool, cursor *string) (string, []interface{}, error) {
...
func (db *PostgreSQLBridge) contractListTop(validatedOnly bool, cursor *string, count int32, list *types.ContractList) error {
	...
	whereClause, args, err := contractListTopFilter(validatedOnly, cursor)
	if err != nil {
	...
	query := "SELECT * FROM contracts " + whereClause + " ORDER BY id DESC LIMIT $1"
...
func (db *PostgreSQLBridge) contractListFilter(validatedOnly bool, cursor *string, count int32, list *types.ContractList) (string, []interface{}, error) {
	...
	if validatedOnly {
		// Filter validated only contracts
		whereClause = fmt.Sprintf("WHERE id %s $1 AND validated IS NOT NULL", ordinalOp)
		args = append(args, list.First)
	} else {
		// Filter all contracts
		whereClause = fmt.Sprintf("WHERE id %s $1", ordinalOp)
		args = append(args, list.First)
	}
...
func (db *PostgreSQLBridge) contractListOptions(count int32) (string, int64) {
	...
	orderByClause := fmt.Sprintf("ORDER BY id %s", sortOrder)
