...
const (
	...
	// fiContractIsDdb marks a contract created through a DDB data-contract transaction.
	// Mirrors types.ContractRow.IsDDB (bson:"is_ddb").
	fiContractIsDdb = "is_ddb"
)

// initContractsCollection initializes the contracts collection with
// indexes and additional parameters needed by the app.
//
// The only index this collection used to carry was {_id:1, orx:-1}. An index prefixed on
// _id cannot serve a query that filters or sorts on `orx` alone, and every contract query
// does exactly that -- the ordinal range scans, the first/last lookups, and the list sort.
// So the index was paid for on every write and used by nothing. Its Unique flag was also
// redundant, since _id is unique by definition.
func (db *MongoDbBridge) initContractsCollection(col *mongo.Collection) {
	...
	// The real access pattern: ordered by ordinal, and range-scanned by ordinal for
	// keyset pagination. Leading with orx is what makes those index scans.
	ix = append(ix, mongo.IndexModel{
		Keys: bson.D{{Key: fiContractOrdinalIndex, Value: -1}},
		Options: &options.IndexOptions{
			Unique: &unique,
		},
	})

	// Serves the validated-contracts list and its total count, which filters on `val`
	// and sorts on `orx`. That count had no index AND no time limit, so it was an
	// unbounded collection scan on every request for that page.
	ix = append(ix, mongo.IndexModel{Keys: bson.D{
		{Key: fiContractSourceValidated, Value: 1},
		{Key: fiContractOrdinalIndex, Value: -1},
	}})

	// Makes "list the DDB data contracts" an index scan rather than a collection scan.
	// Without it the isDDB flag is stored but not usefully queryable.
	ix = append(ix, mongo.IndexModel{Keys: bson.D{
		{Key: fiContractIsDdb, Value: 1},
		{Key: fiContractOrdinalIndex, Value: -1},
	}})

	// create indexes
	if _, err := col.Indexes().CreateMany(context.Background(), ix); err != nil {
		db.log.Errorf("can not create indexes for contracts collection; %s", err.Error())
	}
...
func (db *MongoDbBridge) contractListTotal(col *mongo.Collection, validatedOnly bool, list *types.ContractList) error {
	...
	// Find how many contracts we have. This is bounded: an unbounded CountDocuments
	// here was an uncapped collection scan on every request for the contracts page.
	total, _, err := db.listDocumentsCount(col, &filter)
	if err != nil {
