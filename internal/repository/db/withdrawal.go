...
func (db *MongoDbBridge) initWithdrawalsCollection(col *mongo.Collection) {
	...
	// address + ordinal, so a per-account withdrawal list is one index scan
	ix = append(ix, mongo.IndexModel{Keys: bson.D{
		{Key: types.FiWithdrawalAddress, Value: 1},
		{Key: types.FiWithdrawalOrdinal, Value: -1},
	}})

	// create indexes
	if _, err := col.Indexes().CreateMany(context.Background(), ix); err != nil {
		db.log.Errorf("can not create indexes for withdrawals collection; %s", err.Error())
	}
