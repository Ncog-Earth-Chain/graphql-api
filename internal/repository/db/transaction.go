...
func (db *MongoDbBridge) initTransactionsCollection(col *mongo.Collection) {
	...
	// Block number. lastKnownBlock() sorts the entire transactions collection on this
	// field with no filter; unindexed, that is a full scan plus a blocking sort at
	// startup. It is the fallback that stands between the scanner and a re-scan from
	// genesis, so it is the worst possible moment for it to be slow. It also makes a
	// block -> transactions lookup possible, which had no supporting index at all.
	ix = append(ix, mongo.IndexModel{Keys: bson.D{{Key: fiTransactionBlock, Value: -1}}})
	...
	if _, err := col.Indexes().CreateMany(context.Background(), ix); err != nil {
		db.log.Errorf("can not create indexes for transaction collection; %s", err.Error())
	}
...
func (db *MongoDbBridge) initTrxList(col *mongo.Collection, cursor *string, count int32, filter *bson.D) (*types.TransactionList, error) {
	...
	total, exact, err := db.listDocumentsCount(col, filter)
	if err != nil {
