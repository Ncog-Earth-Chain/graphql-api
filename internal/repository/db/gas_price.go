...
// gasPriceRetention is how long a gas price tick is kept. The collection is written to
// on every tick and only ever read over a recent time range, so unbounded retention buys
// nothing. One year keeps every chart range the API can serve.
const gasPriceRetention = 365 * 24 * time.Hour
...
func (db *MongoDbBridge) initGasPriceCollection(col *mongo.Collection) {
	...
	// TTL on the period end. This collection gets one document per tick forever and is
	// read back over a time range, so without expiry it grows without bound while the
	// old documents can never be useful. Mongo's TTL monitor reaps anything older than
	// the retention window.
	gasPriceTTL := int32(gasPriceRetention / time.Second)
	ix = append(ix, mongo.IndexModel{
		Keys:    bson.D{{Key: types.FiGasPriceTimeTo, Value: 1}},
		Options: options.Index().SetExpireAfterSeconds(gasPriceTTL),
	})

	// create indexes
	if _, err := col.Indexes().CreateMany(context.Background(), ix); err != nil {
		db.log.Errorf("can not create indexes for gas price collection; %s", err.Error())
	}
