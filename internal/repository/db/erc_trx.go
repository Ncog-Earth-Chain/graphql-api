...
func (db *MongoDbBridge) initErc20TrxCollection(col *mongo.Collection) {
	...
	// Compound indexes for the real query shapes. Every ERC-20 list filters on the
	// token-transaction TYPE and then sorts on the ordinal; `tty` previously had no
	// index of any kind, so each of these was a collection scan plus a blocking sort.
	ix = append(ix, mongo.IndexModel{Keys: bson.D{
		{Key: types.FiTokenTransactionType, Value: 1},
		{Key: types.FiTokenTransactionOrdinal, Value: -1},
	}})
	ix = append(ix, mongo.IndexModel{Keys: bson.D{
		{Key: types.FiTokenTransactionType, Value: 1},
		{Key: types.FiTokenTransactionToken, Value: 1},
		{Key: types.FiTokenTransactionOrdinal, Value: -1},
	}})
	ix = append(ix, mongo.IndexModel{Keys: bson.D{
		{Key: types.FiTokenTransactionType, Value: 1},
		{Key: types.FiTokenTransactionSender, Value: 1},
		{Key: types.FiTokenTransactionOrdinal, Value: -1},
	}})
	ix = append(ix, mongo.IndexModel{Keys: bson.D{
		{Key: types.FiTokenTransactionType, Value: 1},
		{Key: types.FiTokenTransactionRecipient, Value: 1},
		{Key: types.FiTokenTransactionOrdinal, Value: -1},
	}})
	...
	if _, err := col.Indexes().CreateMany(context.Background(), ix); err != nil {
		db.log.Errorf("can not create indexes for ERC20 trx collection; %s", err.Error())
	}
