...
//
// This function used to be an empty stub, so the collection carried no index beyond the
// automatic one on _id. Address point-lookups were therefore fine (AccountRow.Address is
// bson:"_id"), but the three token-list queries -- Erc20TokensList, Erc721ContractsList
// and Erc1155ContractsList -- all filter on `type` and sort on {atc:-1, ats:-1} with
// neither field indexed. That is a full collection scan followed by a blocking in-memory
// sort, which Mongo aborts outright once the sort exceeds 32 MB. These are front-page
// queries, so they get slower as the chain grows and then stop working entirely.
func (db *MongoDbBridge) initAccountsCollection(col *mongo.Collection) {
	ix := make([]mongo.IndexModel, 0)

	// Serves filter{type} + sort{atc:-1, ats:-1} as a single index scan with no
	// in-memory sort. Equality field leads, then the sort keys in their sort order.
	ix = append(ix, mongo.IndexModel{Keys: bson.D{
		{Key: fiAccountType, Value: 1},
		{Key: fiAccountTransactionCounter, Value: -1},
		{Key: fiAccountLastActivity, Value: -1},
	}})

	// activity-ordered listing across all account types
	ix = append(ix, mongo.IndexModel{Keys: bson.D{{Key: fiAccountLastActivity, Value: -1}}})

	if _, err := col.Indexes().CreateMany(context.Background(), ix); err != nil {
		db.log.Errorf("can not create indexes for account collection; %s", err.Error())
		return
	}
