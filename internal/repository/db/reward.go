...
func (db *MongoDbBridge) initRewardsCollection(col *mongo.Collection) {
	...
	// address + ordinal: the per-account listing filters on one and sorts on the other,
	// which separate single-field indexes cannot serve without an in-memory sort.
	ix = append(ix, mongo.IndexModel{Keys: bson.D{
		{Key: types.FiRewardClaimAddress, Value: 1},
		{Key: types.FiRewardClaimOrdinal, Value: -1},
	}})

	// create indexes
	if _, err := col.Indexes().CreateMany(context.Background(), ix); err != nil {
		db.log.Errorf("can not create indexes for reward claims collection; %s", err.Error())
	}
