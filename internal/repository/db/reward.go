...
func (db *PostgreSQLBridge) RewardsCount() (uint64, error) {
    var count uint64

    // SQL query to count all rows in the reward_claims table
    query := `SELECT COUNT(*) FROM reward_claims`

    // Execute the query
    err := db.db.QueryRow(query).Scan(&count)
    if err != nil {
        db.log.Errorf("failed to count reward claims; %s", err.Error())
        return 0, err
    }
