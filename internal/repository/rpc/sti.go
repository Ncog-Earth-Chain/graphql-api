...
func (nec *NecBridge) StakerInfo(id *hexutil.Big) (*types.StakerInfo, error) {
	if id == nil {
		return nil, fmt.Errorf("staker ID not given")
	}

	// keep track of the operation
	nec.log.Debugf("loading staker information for staker #%d", id.ToInt().Uint64())

	// instantiate the contract and display its name
	contract, err := contracts.NewStakerInfoContract(nec.sfcConfig.StiContract, nec.eth)
	if err != nil {
		nec.log.Criticalf("failed to instantiate STI contract: %v", err)
		return nil, err
	}
	...
	if err != nil {
		nec.log.Errorf("failed to get the staker #%d information: %v", id.ToInt().Uint64(), err)
		return nil, err
	}

	// var url string
	if len(stUrl) == 0 {
		nec.log.Debugf("no information for staker #%d", id.ToInt().Uint64())
		return nil, nil
	}

	// try to download JSON for the info
	return nec.downloadStakerInfo(stUrl)
}

// downloadStakerInfo tries to download staker information from the given URL address.
func (nec *NecBridge) downloadStakerInfo(stUrl string) (*types.StakerInfo, error) {
	// log what we are about to do
	nec.log.Debugf("downloading staker info address [%s]", stUrl)
	...
	if err != nil {
		nec.log.Errorf("can not request given staker info at [%s]; %s", stUrl, err.Error())
		return nil, err
	}

	// be honest, set agent
	req.Header.Set("User-Agent", "NcogEarthChain GraphQL API Server")

	// process the request
	res, err := cl.Do(req)
	if err != nil {
		nec.log.Errorf("can not download staker info from [%s]; %s", stUrl, err.Error())
		return nil, err
	}
	...
	if err != nil {
		nec.log.Errorf("can not read staker info response from [%s]; %s", stUrl, err.Error())
		return nil, err
	}
	...
	if err != nil {
		nec.log.Errorf("can not decode staker info [%s]; %s", stUrl, err.Error())
		nec.log.Debugf("Received: %s", body)
		return nil, err
	}

	// do we have anything?
	if !nec.isValidStakerInfo(&info) {
		nec.log.Errorf("invalid staker info content [%s]", stUrl)
		return nil, err
	}

	nec.log.Debugf("found staker [%s]", *info.Name)
	return &info, nil
}

// isValidStakerInfo check if the staker information is valid and can be used.
func (nec *NecBridge) isValidStakerInfo(info *types.StakerInfo) bool {
	// name must be available
	if nil == info.Name || 0 == len(*info.Name) || !stiNameCheckRegex.Match([]byte(*info.Name)) {
		nec.log.Error("staker name not valid")
		return false
	}

	// check the logo URL
	if !isValidStakerInfoUrl(info.LogoUrl, true) {
		nec.log.Error("staker logo URL not valid")
		return false
	}

	// check the website
	if !isValidStakerInfoUrl(info.Website, false) {
		nec.log.Error("staker website URL not valid")
		return false
	}

	// check the contact URL
	if !isValidStakerInfoUrl(info.Contact, false) {
		nec.log.Error("staker contact URL not valid")
		return false
	}
	return true
}
