...
func (nec *NecBridge) GasPrice() (hexutil.Big, error) {
	// keep track of the operation
	nec.log.Debugf("checking current gas price")

	// call for data
	var price hexutil.Big
	var try uint8
	for {
		err := nec.rpc.Call(&price, "eth_gasPrice")
		if err != nil {
		...
		// all attempts failed
		if try > 2 {
			nec.log.Errorf("can not get reasonable gas price from the backend server")
...
func (nec *NecBridge) GasEstimate(trx *struct {
	From  *common.Address
	To    *common.Address
	Value *hexutil.Big
	Data  *string
...
func (nec *NecBridge) GasEstimateWithBlock(trx *struct {
	From  *common.Address
	To    *common.Address
	Value *hexutil.Big
	Data  *string
	...
	var val hexutil.Uint64
	err := nec.rpc.Call(&val, "eth_estimateGas", trx, BlockTypeLatest)
	if err != nil {
		// return error
		nec.log.Errorf("can not estimate gas; %s", err.Error())
		return nil, err
