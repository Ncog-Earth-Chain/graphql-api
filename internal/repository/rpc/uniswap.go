...
func (nec *NecBridge) NativeTokenAddress() (*common.Address, error) {
	// get the router contract if possible
	contract, err := contracts.NewUniswapRouter(nec.uniswapConfig.Router, nec.eth)
	if err != nil {
		nec.log.Errorf("Uniswap router contract not found; %s", err.Error())
		return nil, err
	}
	...
	if err != nil {
		nec.log.Errorf("Native token address not available; %s", err.Error())
		return nil, err
	}
...
func (nec *NecBridge) UniswapPair(tokenA *common.Address, tokenB *common.Address) (*common.Address, error) {
	// get the router contract if possible
	contract, err := contracts.NewUniswapFactory(nec.uniswapConfig.Core, nec.eth)
	if err != nil {
		nec.log.Errorf("Uniswap factory contract not found; %s", err.Error())
		return nil, err
	}
	...
	if err != nil {
		nec.log.Errorf("Uniswap pair not found for tokens %s and %s; %s", tokenA.String(), tokenB.String(), err.Error())
		return nil, err
	}
...
func (nec *NecBridge) UniswapPairs(whiteListedOnly bool) ([]common.Address, error) {
	// get the router contract if possible
	contract, err := contracts.NewUniswapFactory(nec.uniswapConfig.Core, nec.eth)
	if err != nil {
		nec.log.Errorf("Uniswap factory contract not found; %s", err.Error())
	...
	if err != nil || length == nil {
		nec.log.Error("Uniswap pairs array length not available")
		return nil, err
	}
	...
	for i := uint64(0); i < length.Uint64(); i++ {
		...
		if err != nil {
			nec.log.Errorf("error loading Uniswap pair; %s", err.Error())
			continue
		}

		// check the pair (is it on the white list?)
		// add the address to the list, if it's ok
		if !whiteListedOnly || nec.isUniswapPairWhitelisted(&adr) {
...
func (nec *NecBridge) isUniswapPairWhitelisted(pair *common.Address) bool {
	// decode the pair address
	pairAddr := pair.String()

	// check the pair address against all the white listed pairs in config
	for _, addr := range nec.uniswapConfig.PairsWhiteList {
...
func (nec *NecBridge) UniswapQuoteInput(
	amountA hexutil.Big,
	...
	if err != nil {
		nec.log.Errorf("can not calculate Uniswap quote; %s", err.Error())
		return hexutil.Big{}, err
	}
...
func (nec *NecBridge) UniswapAmountsOut(amountIn hexutil.Big, tokens []common.Address) ([]hexutil.Big, error) {
	// get the router contract if possible
	contract, err := contracts.NewUniswapRouter(nec.uniswapConfig.Router, nec.eth)
	if err != nil {
		nec.log.Errorf("Uniswap router contract not found; %s", err.Error())
	...
	return nec.uniswapLoadAmounts(contract.GetAmountsOut, (*big.Int)(&amountIn), tokens)
}
...
func (nec *NecBridge) UniswapAmountsIn(amountOut hexutil.Big, tokens []common.Address) ([]hexutil.Big, error) {
	// get the router contract if possible
	contract, err := contracts.NewUniswapRouter(nec.uniswapConfig.Router, nec.eth)
	if err != nil {
		nec.log.Errorf("Uniswap router contract not found; %s", err.Error())
	...
	return nec.uniswapLoadAmounts(contract.GetAmountsIn, (*big.Int)(&amountOut), tokens)
}

// uniswapLoadAmounts loads a list of swap amounts using the given loader function
// of the Uniswap router contract.
func (nec *NecBridge) uniswapLoadAmounts(
	...
	if err != nil {
		nec.log.Errorf("can not load swap amounts; %s", err.Error())
		return nil, err
	}
...
func (nec *NecBridge) UniswapTokens(pair *common.Address) ([]common.Address, error) {
	// get the pair contract if possible
	contract, err := contracts.NewUniswapPair(*pair, nec.eth)
	if err != nil {
		nec.log.Errorf("Uniswap pair %s not found; %s", pair.String(), err.Error())
		return nil, err
	}
	...
	if err != nil {
		nec.log.Errorf("Uniswap pair %s token A not found; %s", pair.String(), err.Error())
		return nil, err
	}
	...
	if err != nil {
		nec.log.Errorf("Uniswap pair %s token B not found; %s", pair.String(), err.Error())
		return nil, err
	}
...
func (nec *NecBridge) UniswapReserves(pair *common.Address) ([]hexutil.Big, error) {
	// get the reserves record from the contract
	rs, err := nec.uniswapReservesRecord(pair)
	if err != nil {
		nec.log.Errorf("Uniswap pair %s reserves not available; %s", pair.String(), err.Error())
		return nil, err
	}
...
func (nec *NecBridge) UniswapReservesTimeStamp(pair *common.Address) (hexutil.Uint64, error) {
	// get the reserves record from the contract
	rs, err := nec.uniswapReservesRecord(pair)
	if err != nil {
		nec.log.Errorf("Uniswap pair %s reserves not available; %s", pair.String(), err.Error())
...
func (nec *NecBridge) UniswapCumulativePrices(pair *common.Address) ([]hexutil.Big, error) {
	// get the pair contract if possible
	contract, err := contracts.NewUniswapPair(*pair, nec.eth)
	if err != nil {
		nec.log.Errorf("Uniswap pair %s not found; %s", pair.String(), err.Error())
	...
	if err != nil {
		nec.log.Errorf("Uniswap pair %s cumulative price A not available; %s", pair.String(), err.Error())
		return nil, err
	}
	...
	if err != nil {
		nec.log.Errorf("Uniswap pair %s cumulative price B not available; %s", pair.String(), err.Error())
		return nil, err
	}
...
func (nec *NecBridge) uniswapReservesRecord(pair *common.Address) (*struct {
	Reserve0           *big.Int
	Reserve1           *big.Int
...
func (nec *NecBridge) UniswapLastKValue(pair *common.Address) (hexutil.Big, error) {
	// get the pair contract if possible
	contract, err := contracts.NewUniswapPair(*pair, nec.eth)
	if err != nil {
		nec.log.Errorf("Uniswap pair %s not found; %s", pair.String(), err.Error())
	...
	if err != nil {
		nec.log.Errorf("Uniswap pair %s coefficient K not available; %s", pair.String(), err.Error())
		return hexutil.Big{}, err
	}
...
func (nec *NecBridge) UniswapPairContract(pairAddres *common.Address) (*contracts.UniswapPair, error) {
	contract, err := contracts.NewUniswapPair(*pairAddres, nec.eth)
	if err != nil {
		nec.log.Errorf("Uniswap pair contract %s not found; %s", pairAddres.String(), err.Error())
		return nil, err
	}
	return contract, nil
}
...
func (nec *NecBridge) UniswapFactoryContract() (*contracts.UniswapFactory, error) {
	// get the router contract if possible
	contract, err := contracts.NewUniswapFactory(nec.uniswapConfig.Core, nec.eth)
	if err != nil {
		nec.log.Errorf("Uniswap factory contract not found; %s", err.Error())
