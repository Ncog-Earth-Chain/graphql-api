...
func (nec *NecBridge) FLendGetLendingPool() (*contracts.ILendingPool, error) {
	// get the lending pool contract
	lp, err := contracts.NewILendingPool(nec.fLendCfg.lendigPoolAddress, nec.eth)
	if err != nil {
		nec.log.Errorf("Can not get lending pool contract on address %s; %s", nec.fLendCfg.lendigPoolAddress.String(), err.Error())
		return nil, err
	}
	return lp, nil
}
...
func (nec *NecBridge) FLendGetLendingPoolReserveData(assetAddress *common.Address) (*types.ReserveData, error) {

	// get the lending pool contract
	lp, err := nec.FLendGetLendingPool()
	if err != nil {
		nec.log.Errorf("Can not access lending pool %s", err.Error())
		return nil, err
	}

	rd, err := lp.GetReserveData(&bind.CallOpts{}, *assetAddress)
	if err != nil {
		nec.log.Errorf("Cannot get reserve data for asset %s: %s", assetAddress.String(), err.Error())
		return nil, err
	}
...
func (nec *NecBridge) FLendGetReserveList() ([]common.Address, error) {
	...
	if err != nil {
		nec.log.Errorf("Cannot get reserves list: %s", err.Error())
		return nil, err
	}
	return rl, nil
}
...
func (nec *NecBridge) FLendGetUserAccountData(userAddress *common.Address) (*types.FLendUserAccountData, error) {
	...
	if err != nil {
		nec.log.Errorf("Cannot get user account data for address %s: %s", userAddress.String(), err.Error())
		return nil, err
	}

	uc, err := lp.GetUserConfiguration(&bind.CallOpts{}, *userAddress)
	if err != nil {
		nec.log.Errorf("Cannot get user account configuration data for address %s: %s", userAddress.String(), err.Error())
		return nil, err
	}
...
func (nec *NecBridge) FLendGetUserDepositHistory(userAddress *common.Address, assetAddress *common.Address) ([]*types.FLendDeposit, error) {
	// create user filter
	...
	if err != nil {
		nec.log.Errorf("can not filter lending pool deposit logs: %s", err.Error())
		return nil, err
	}
	...
	for fdi.Next() {
		...
		blk, err := nec.BlockByHash(&blkHash)
		if err != nil {
			nec.log.Errorf("fLend block with hash %s was not found: %s", blkHash, err.Error())
			continue
		}
