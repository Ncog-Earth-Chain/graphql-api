...
//go:generate tools/abigen.sh --abi ./contracts/abi/wnec.abi --pkg contracts --type ErcWrappedNec --out ./contracts/erc20wnec_token.go

// Erc20Name provides information about the name of the ERC20 token.
func (nec *NecBridge) Erc20Name(token *common.Address) (string, error) {
	// connect the contract
	contract, err := contracts.NewERCTwenty(*token, nec.eth)
	if err != nil {
		nec.log.Errorf("can not contact ERC20 contract; %s", err.Error())
		return "", err
	}
...
func (nec *NecBridge) Erc20Symbol(token *common.Address) (string, error) {
	// connect the contract
	contract, err := contracts.NewERCTwenty(*token, nec.eth)
	if err != nil {
		nec.log.Errorf("can not contact ERC20 contract; %s", err.Error())
...
func (nec *NecBridge) Erc20Decimals(token *common.Address) (int32, error) {
	// connect the contract
	contract, err := contracts.NewERCTwenty(*token, nec.eth)
	if err != nil {
		nec.log.Errorf("can not contact ERC20 contract; %s", err.Error())
...
func (nec *NecBridge) Erc20BalanceOf(token *common.Address, owner *common.Address) (hexutil.Big, error) {
	// connect the contract
	contract, err := contracts.NewERCTwenty(*token, nec.eth)
	if err != nil {
		nec.log.Errorf("can not contact ERC20 contract; %s", err.Error())
	...
	if err != nil {
		nec.log.Errorf("can not ERC20 %s balance for %s; %s", token.String(), owner.String(), err.Error())
		return hexutil.Big{}, err
	}
	...
	if val == nil {
		nec.log.Errorf("no balance available for ERC20 %s, owner %s", token.String(), owner.String())
		val = new(big.Int)
	}
...
func (nec *NecBridge) Erc20Allowance(token *common.Address, owner *common.Address, spender *common.Address) (hexutil.Big, error) {
	...
	// return zero even for unknown owners, but let's be sure here
	if val == nil {
		nec.log.Errorf("no allowance available for ERC20 %s, owner %s", token.String(), owner.String())
		val = new(big.Int)
	}
...
func (nec *NecBridge) Erc20TotalSupply(token *common.Address) (hexutil.Big, error) {
	// connect the contract
	contract, err := contracts.NewERCTwenty(*token, nec.eth)
	if err != nil {
		nec.log.Errorf("can not contact ERC20 contract; %s", err.Error())
	...
	if err != nil {
		nec.log.Errorf("can not get ERC20 %s total supply; %s", token.String(), err.Error())
		return hexutil.Big{}, err
	}

	// make sure we always have a value; at least zero
	if val == nil {
		nec.log.Errorf("no supply available for ERC20 %s", token.String())
		val = new(big.Int)
	}
