...
// ContractPost extract a smart contract information by account address, if available.
// func (p *proxy) ContractPost(addr *common.Address) (*types.Contract, error) {
// 	// try cache first
// 	sc := p.cache.PullContract(addr)

// 	// we still don't know the contract? call the pdDB for that
// 	if sc == nil {
// 		var err error
// 		sc, err = p.pdDB.Contract(addr)
// 		if err != nil {
...
func (p *proxy) ValidateContract(sc *types.Contract) error {
	...
	for name, detail := range contracts {
		...
		if match {
			...
			if err := p.pdDB.UpdateContract(sc); err != nil {
...
// provided source code. If successful, the contract information
// is updated the the repository.
// func (p *proxy) ValidateContractPost(sc *types.Contract) error {
// 	// get the byte code of the actual contract
// 	tx, err := p.Transaction(&sc.TransactionHash)
// 	if err != nil {
// 		p.log.Errorf("can not get contract deployment transaction; %s", err.Error())
...
// 			// write update to the database
// 			if err := p.pdDB.UpdateContract(sc); err != nil {
// 				p.log.Errorf("contract validation failed due to pdDB error; %s", err.Error())
// 				return err
// 			}
...
func (p *proxy) StoreContract(con *types.Contract) error {
	...
	isUpdate := p.pdDB.IsContractKnown(&con.Address)

	// do the add/update op
	if err := p.pdDB.AddContract(con); err != nil {
...
// StoreContract adds new contract into the repository.
// func (p *proxy) StoreContractPost(con *types.Contract) error {
// 	// is the a known contract which will be updated?
// 	isUpdate := p.pdDB.IsContractKnown(&con.Address)
