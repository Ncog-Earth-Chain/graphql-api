...
func (app *apiServer) makeHttpServer() {
	...
	// The write deadline must outlast the resolver deadline. http.TimeoutHandler
	// can only send its "Service timeout." response while it still owns the
	// connection; if the server's write deadline fires first, the socket is cut
	// and the client sees a truncated response instead of a clean timeout.
	if app.cfg.Server.WriteTimeout <= app.cfg.Server.ResolverTimeout {
		adjusted := app.cfg.Server.ResolverTimeout + 5
		app.log.Warningf(
			"server.write_timeout (%ds) must exceed server.resolver_timeout (%ds); raising it to %ds",
			app.cfg.Server.WriteTimeout, app.cfg.Server.ResolverTimeout, adjusted)
		app.cfg.Server.WriteTimeout = adjusted
	}
...
func (app *apiServer) setupHandlers(mux *http.ServeMux) {
	...
	if app.cfg.Server.GraphiEnabled {
		app.log.Warningf("GraphiQL IDE is ENABLED and unauthenticated on /graphi")
